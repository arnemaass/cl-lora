...
def check_base_sd_consistency(peft_model, base_sd, verbose=True):
    ...
    automatically handles the additional `.base_layer` wrapper in PEFT models.
    """
    ...
    for k, v in base_sd.items():
        ...
        if peft_key is None:
            if k == "fc.weight" or k == "fc.bias":
                continue
            if verbose:
                print(f"[Missing] {k} ↛ (Can't find in peft_model)")
            all_match = False
            mismatch_count += 1
            continue
        ...
        try:
            if not torch.allclose(model_sd[peft_key], v, atol=1e-6):
                if verbose:
                    print(f"[Mismatch] {k} ≠ {peft_key}")
                    print(f"  base_sd  shape: {v.shape}")
                    print(f"  peft_sd  shape: {model_sd[peft_key].shape}")
                    print(f"  max |Δ|  : {(model_sd[peft_key] - v).abs().max().item():.3e}")
                all_match = False
                mismatch_count += 1
            checked_count += 1
        except Exception as e:
            print(f"[Error] Error comparing {k} : {e}")
            all_match = False
            mismatch_count += 1

    # === Summary ===
    print("\n=== Summary ===")
    print(f"Total parameters checked : {checked_count}")
    print(f"Total mismatches/missing : {mismatch_count}")
    if all_match:
        print("✅ All base weights loaded correctly!")
    else:
        print("❌ Uncorrect base weights loaded. See above log.")
...
def check_peft_sd_consistency(peft_model, peft_sd, verbose=True):
    ...
    for k, v in peft_sd.items():
        ...
        try:
            if not torch.allclose(model_sd[peft_key], v, atol=1e-6):
                if verbose:
                    print(f"[Mismatch] {k} ≠ {peft_key}")
                    print(f"  peft_sd shape: {v.shape}")
                    print(f"  model_sd shape: {model_sd[peft_key].shape}")
                    print(f"  max |Δ|       : {(model_sd[peft_key] - v).abs().max().item():.3e}")
                all_match = False
                mismatch_count += 1
            checked_count += 1
    ...
    print(f"Total mismatches/missing : {mismatch_count}")
    if all_match:
        print("✅ All lora LoRA keys match！")
    else:
        print("❌ LoRA keys missing or mismatch, see above log.")

    return all_match

def extract_model_info(LoRA_ViT_model: torch.nn.Module):
        """Extract and print model info from LoRA_ViT_model"""
        ...
        print("\n=== PyTorch Model Info ===")
        print(f"Type: {type(pytorch_model)}")
        ...
        print("\n=== State Dict Keys ===")
        print("PyTorch model keys:", list(pytorch_state_dict.keys())[:10], "...")
...
def load_peft_model(base_sd, lora_head):
    ...
    _ = base.load_state_dict(base_sd, strict=False)
    print(_)
    cfg = LoraConfig(
    ...
    # load peft model (change prefix first)
    peft_model = peft_model.to(device)
    lora_head = {k: v.to(device) for k, v in lora_head.items()}
    #peft_sd_prefixed = {f"base_model.model.{k}": v for k, v in lora_head.items()}
...
def load_peft_models(lora_heads: List[Dict[str, torch.Tensor]], LoRA_ViT_model: torch.nn.Module):
    ...
    #base_sd_prefixed = {f"base_model.model.{k}": v for k, v in base_sd.items()}
    # create base_sd  
    cache = {}
    first_dict = None
    for idx, lora_head in enumerate(lora_heads):
        print(f"> Loading LoRA head {idx}")

"""
lorahub_learning.py
LoRAHub learning and merging utilities for continual learning scenarios.

This module provides functions to:
1. Merge LoRA adapters incrementally in continual learning settings
2. Merge all LoRA adapters from scratch for multi-task scenarios
3. Handle classifier head merging with weighted averaging
4. Manage model parametrization and weight baking

The module supports two main merging strategies:
- Continual merging: Incrementally merge new task adapters with previous results

Dependencies
------------
• torch, torch.nn, torch.nn.functional
• transformers, tqdm, logging
• Custom algorithms module for LoRA optimization
"""
...
def _split_weight_bias(
    ...
    """
    Extract the weight and optional bias from a state dictionary.
    
    Args:
        head: State dictionary containing weight and optionally bias tensors
    
    Returns:
        tuple: (weight, bias) where bias may be None
    
    Raises:
        ValueError: If no 2D weight tensor is found in the state dict
    """
    try:
...
def _cleanup_and_bake_weights(model: nn.Module) -> None:
    """
    Remove all parametrizations from the model, baking the final weights.
    The monkey-patched forward method is intentionally NOT restored, so the
    model continues to use the new classifier head.

    Args:
        model: The model to clean up and bake weights into
    """
...
def LoraHubMerge_continual(
    ...
    """
    Incrementally merge a new task's LoRA adapter into a previously merged adapter.
    ...
    - **LoRA Merging**: Learns coefficients to combine the previous merged LoRA adapter
      and the new task's LoRA adapter using LoRAHub optimization.
    ...
    - **Continual Learning**: Maintains performance on previous tasks while adapting to new ones.

    Args:
        pl_model: The base model (e.g., a Pytorch Lightning module)
        train_loader_old: DataLoader for the old task/replay data
        train_loader_new: DataLoader for the new task data
        lora_heads: A list of 2 LoRA state dicts:
            [0]: The previously merged LoRA adapter
            [1]: The new task's LoRA adapter
        classifier_heads: A list of 2 classifier state dicts:
            [0]: The previously merged classifier head
            [1]: The new task's classifier head
        mode: Merging mode. "learnable" for trainable alpha, "static" for uniform alpha
        num_epochs: Number of epochs to train the merging coefficients
        lr: Learning rate for the optimizer
        current_task: The number of the current task (e.g., 2, 3, ...). Used for weighting
    ...
    # Create a subset of data for LoRAHub optimization
    ...
    # Train the classifier on combined old and new task data
    ...
    # 6. Build merged weights from trained parameters using LoRAHub
    # Call LoRA Hub merge function to optimize LoRA weight combinations
    ...
    # Extract the final merged classifier
    merged_classifier = {}
...
def LoraSoupsMerge_from_scratch(
    ...
    """
    Merge all task adapters from scratch to create a single unified adapter.
    ...
    - **LoRA Merging**: Learns coefficients to combine all original LoRA adapters provided
      using parametrization-based approach.
    - **Classifier Merging**: Creates a new classifier by uniformly averaging the weights
      of all original classifier heads provided.
    - **From-Scratch Approach**: Merges all original adapters rather than incremental merging.

    Args:
        pl_model: The base model (e.g., a Pytorch Lightning module)
        train_loader: A single DataLoader containing samples from all tasks
        lora_heads: A list of T LoRA state dicts, one for each original task adapter from task 1 to T
        classifier_heads: A list of T classifier state dicts, one for each original task
        mode: Merging mode. "learnable" for trainable alpha, "static" for uniform alpha
        num_epochs: Number of epochs to train the merging coefficients
        lr: Learning rate for the optimizer
        current_task: The number of the current task (e.g., 2, 3, ...). Used for logging
    ...
    # Train the merged model on all task data
    for ep in range(num_epochs):

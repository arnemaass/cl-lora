...
from .video_vit import VisionTransformer as SViT
...
class LoRA_SViT(nn.Module):
    def __init__(self, vit_model: SViT, r: int, alpha: int, num_classes: int = 0, lora_layer=None):
        super(LoRA_SViT, self).__init__()

        assert r > 0
        assert alpha > 0
        base_vit_dim = vit_model.blocks[0].attn.q.in_features
        dim = base_vit_dim
        if lora_layer:
        ...
        for t_layer_i, blk in enumerate(vit_model.blocks):
            ...
            blk.attn.q = _LoRALayer(w_q_linear, w_a_linear_q, w_b_linear_q, r, alpha)
            blk.attn.v = _LoRALayer(w_v_linear, w_a_linear_v, w_b_linear_v, r, alpha)

        self.reset_parameters()
        self.lora_vit = vit_model

        if num_classes > 0:
            self.lora_vit.fc = nn.Linear(vit_model.embed_dim, num_classes)
            torch.nn.init.normal_(self.lora_vit.fc.weight, std=0.02)

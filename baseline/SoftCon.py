...
def load_model(r=4):
    ...
    model_vitb14 = load_softcon_encoder(
        model_vitb14, "/faststorage/softcon/pretrained/B13_vitb14_softcon_enc.pth"
    )
...
def train_model_replay(lora_model, train_loader, val_loader, epochs=25, lr=1e-4):
    ...
    with open(log_file, mode="a", newline="") as f:
        ...
        for epoch in range(epochs):
            ...
            print(
                f"Epoch {epoch + 1}/{epochs}, Train Loss: {avg_train_loss:.4f}, Train Acc: {train_acc:.2f}%, Val Loss: {avg_val_loss:.4f}, Val Acc: {val_acc:.2f}%"
            )
            writer.writerow(
...
def load_country_train_val(
    ...
    n_samples: int,
    seed: int = None,
    train_frac: float = 0.8,  # Default train/val split fraction
    img_size=(14, 224, 224),
    include_snowy=False,
    include_cloudy=False,
    split="train",
    frac=None,  # Add frac argument for compatibility
):
    ...
    # Use frac if provided, otherwise default to train_frac
    split_at = int(n_samples * (frac if frac is not None else train_frac))
